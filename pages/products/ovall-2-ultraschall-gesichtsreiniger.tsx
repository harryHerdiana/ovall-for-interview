...
import ProductStage from '@component/ProductStage/ProducStage2'
...
const ProductBundlePage: React.FC<IProductPage> = (props: IProductPage & IDefaultProps) => {
  const {
    seoTags,
    appProps,
  ...
  const [variantSku, setVariantSku] = React.useState(productBundle.variants[0].sku)
  const variant =
    productBundle.variants.find((v) => v.sku === variantSku) || productBundle.variants[0]
...
export const getStaticProps: GetStaticProps = async (context) => {
  ...
  const data = await pageDataService.productBundle()
